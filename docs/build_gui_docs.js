...
doc("3 - Technical Reference", [
  ...
  TABLE(["Field", "Range", "Notes"], [
    ...
    ["Fatigue frequency", "1 to 5 Hz, whole", "a true square wave needs <= 0.25 Hz (see below)"],
  ...
  P("A fatigue square wave presses the actuator between the lower and upper force and back, over and over. The rod cannot move infinitely fast, so above a certain frequency the press never reaches its bounds or never holds flat - it rounds into a triangle and then shrinks in amplitude. A frequency sweep on the rig characterizes exactly where that happens for this build."),
  H3("Deviation metric"),
  P([{ t: "Square-wave deviation: ", b: true }, { t: "the RMS error between the achieved and the commanded (ideal square) position, as a percent of the commanded amplitude. 0 percent is a perfect square; higher means more rounding and lag. The usable limit is the fastest frequency whose deviation stays within tolerance and that still reaches its bounds." }]),
  H3("Measured result (frequency sweep)"),
  TABLE(["Frequency", "Square-wave shape"], [
    [{ t: "<= 0.25 Hz", b: true, color: GREEN }, { t: "usable - reaches the bounds and holds roughly flat", color: GREEN }],
    [{ t: "0.5 Hz", b: true, color: AMBER }, { t: "deviates significantly - edges round off, the flats shrink", color: AMBER }],
    [{ t: ">= 1 Hz", b: true, color: RED }, { t: "clearly not a square any more", color: RED }],
  ], [2200, 7160], false),
  NOTE("Values from the frequency sweep. The sweep is dense below 0.5 Hz (0.1, 0.15, 0.2, 0.25 ... 0.5 Hz); rerun on the rig with tools/zaber_square_wave_characterization.py and paste characterization_curve.svg (deviation, amplitude fidelity, and tracking lag vs frequency, with the usable limit marked) below this note."),
  ...
  P([{ t: "For a faithful square wave, cap the fatigue frequency at " }, { t: "0.25 Hz", b: true, color: GREEN }, { t: ". Above it the option no longer produces a square (it degrades to a rounded triangle), so allowing higher would silently hand back a shape the operator did not ask for." }]),
  SHADE([{ t: "Tradeoff: ", b: true }, { t: "at 0.25 Hz the default 28,800-cycle test runs about 32 hours. If a true square is required, reduce the cycle count or accept the duration. If only the cycling matters (a rounded shape is acceptable), a higher frequency - or a sine / triangle waveform, which the actuator tracks better - is fine." }], "FBF4E6"),
  P([{ t: "OPEN ITEM: ", b: true, color: AMBER }, { t: "confirm with the sensor team whether the fatigue test needs a true square (=> 0.25 Hz cap) or tolerates a rounded cycle (=> keep the higher field limit), and refine the exact number from the dense rig sweep.", color: AMBER }]),
