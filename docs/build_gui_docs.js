...
doc("2 - Operator Guide", [
  ...
  P([{ t: "Stops and homes on the 33 N force ceiling, a sudden force jump (metal contact), or the travel limit. On a Zaber disconnect it stops safely in place (no homing - the position is unknown once comms are lost); the operator reconnects with the Zaber Launcher and presses Start (or a jog) to resume. " }, { t: "The motion and run buttons move a real actuator into a load cell; never click them casually.", b: true, color: RED }]),
  ...
  P("Force ceiling (confirm stop and home); disconnect (unplug mid-run, confirm safe stop in place with no homing, then reconnect via the Zaber Launcher and press Start to resume); out-of-range input (confirm snap-to clamps and names the limit)."),
  H3("Sign-off"),
...
doc("3 - Technical Reference", [
  ...
  TABLE(["Item", "Value", "Behavior"], [
    ...
    ["Lost connection", "-", "safe stop without homing; reconnect via the Zaber Launcher, then Start re-initializes (no auto re-home)"],
...
doc("5 - Test Plan and Verification", [
  ...
  TABLE(["Safety case", "Method", "Expected", "Result / Evidence"], [
    ...
    ["Zaber disconnect", "unplug actuator mid-run", "safe stop without homing; reconnect via the Zaber Launcher, then Start/jog re-initializes (no auto re-home)", ""],
